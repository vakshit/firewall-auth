...
def start_func():
    ...
    try:
        ...
        logger.info(
            "Exception |%s| while trying to log in. Retrying in %d seconds."
            % (e, ERROR_RETRY_SECS)
        )
    ...
    if loginstate == LoginState.AlreadyLoggedIn:
        logger.info(
            "You're already logged in (response code %d). Retrying in %d seconds."
...
def logged_in_func(keepaliveurl):
    ...
    try:
        ...
        logger.info(
            "The keepalive URL %s doesn't work. Attempting to log in again."
            % keepaliveurl.geturl()
        )
        return (FirewallState.Start, 0, None)
    except (http.client.HTTPException, socket.error) as e:
        logger.info(
            "Exception |%s| while trying to keep alive. Retrying in %d seconds."
            % (e, ERROR_RETRY_SECS)
        )
...
def run_state_machine():
    ...
    def atexit_logout():
        ...
        if state == FirewallState.LoggedIn:
            url = args[0]
            logouturl = urllib.parse.ParseResult(
                url.scheme, url.netloc, "/logout", url.params, url.query, url.fragment
            )
            try:
...
def login():
    ...
    try:
        ...
        # conn = http.client.HTTPSConnection("172.31.1.251:1003")
        conn.request("GET", "/")
        response = conn.getresponse()
    ...
    try:
        ...
        data = authResponse.read().decode("utf-8")
    finally:
        authconn.close()
    ...
    try:
        ...
        aliveURL = postResponse.getheader("Location")
        # postData = postResponse.read().decode("utf-8")
    finally:
        postconn.close()
